...
pub fn verify_secret(secret: &str) -> bool {
    ...
    let input_token: String = input()
        ...
        .add_err_test(|m: &String| syntatic_validation_google_token(m), "Error: the format is not respected (only 6 numbers) " )
        .get();
...
pub fn change_two_factors(
    username: &str,
    two_factors_is_enabled: bool,
) -> Result<bool, Errors> {
...
fn enable_two_factors(username: &str) -> Result<bool, Errors> {
    ...
    let qr_code = google_auth.qr_code_url(secret.as_str(), "qr_code", "sec_labo2_account",400 ,400 ,ErrorCorrectionLevel::High);
